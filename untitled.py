...
class RNN_arch_2_final(nn.Module):
	...
	def forward(self, x, cue, hc1, hc2, hc3, hc4):
		...
		return output_seq, hc1, hc2, hc3, hc4, output_seq1, output_seq2, cue_arr, feed1_arr, feed2_arr, out_f_arr, out1, out2
...
class decoder(nn.Module):
	...
	def forward(self, x):
		output = torch.empty((16, self.batch_size, 8))
		for i in range(16):
			out = self.l1(x[i])
			output[i] = out
		return output
...
def train():
	...
	# model.rnn2_h2o.weight.data = model2.fc1.weight.data

	model.load_state_dict(torch.load("balle_balle_ho_gayaa.pt", map_location = device))
	...
	for i in range(100000):
		data1 = generate_contrast_data_2outputs()
		temp = [data1[0][0], data1[1][0]]
		a.append(temp)

	b = []
	for i in range(100000):
		data1 = generate_motion_data_2outputs()
		temp = [data1[0][0], data1[1][1]]
		b.append(temp)
	...
	while(count0<100):
		...
		for x,y in train_loader:
			...
			_, _, _, _, _, out1, _, _, _, _, _ = model(x.float(), cue, hc1.float(), hc2.float(), hc3.float(), hc4.float())
		...
		for val_x, val_y in valid_loader:
			...
			_, _, _, _, _, val_out1, _, _, _, _, _ = model(val_x.float(), cue, hc1.float(), hc2.float(), hc3.float(), hc4.float())
			val_output = model1(val_out1.float())
		...
		torch.save(model1.state_dict(), "cn_cd_decoder.pt")
	...
	while(count0<150):
		...
		torch.save(model1.state_dict(), "cn_md_decoder.pt")
	...
	while(count0<100):
		...
		for x,y in train_loader:
			...
			_, _, _, _, _, _, out1, _, _, _, _ = model(x.float(), cue, hc1.float(), hc2.float(), hc3.float(), hc4.float())
		...
		for val_x, val_y in valid_loader:
			...
			_, _, _, _, _, _, val_out1, _, _, _, _ = model(val_x.float(), cue, hc1.float(), hc2.float(), hc3.float(), hc4.float())
			val_output = model1(val_out1.float())
			val_output = val_output.contiguous().view(3200, -1)
			valid_loss = criterion(val_output, val_y_reshaped)
			valid_loss = valid_loss.item()
		...
		torch.save(model1.state_dict(), "mn_md_decoder.pt")
	...
	while(count0<150):
		...
		torch.save(model1.state_dict(), "mn_cd_decoder.pt")
	...
	# 	torch.save(model1.state_dict(), "f0_cn_md_decoder.pt")
	...
	# 	torch.save(model1.state_dict(), "f0_mn_md_decoder.pt")
	...
	# 	torch.save(model1.state_dict(), "f0_cn_cd_decoder.pt")
	...
	# 	torch.save(model1.state_dict(), "f0_mn_cd_decoder.pt")
...
def predict_new():
	...
	# for param in model1.parameters():
	# 	param.requires_grad = False
	# model1.load_state_dict(torch.load("cn_cd_decoder.pt", map_location = device))
	...
	for i in range(1000):
		...
		# temp = generate_motion_data_2outputs()
		# data = [temp[0][0], temp[1][1]]
		...
		# output, _ = model1(x.float(), hc1.float())
		_, _, _, _, _, _, _, _, _, _, _, output, _ = model(x.float(), 0, hc1.float(), hc2.float(), hc3.float(), hc4.float())
