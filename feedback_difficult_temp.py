...
def add_noise_1(arr):
	...
	Uniform dostribution and with a probability of 0.45
	"""
	mini = np.amin(arr)
	maxi = np.amax(arr)

	for i in range(arr.shape[0]):
		choice = random.choices([0, 1], [.55, .45])
		epsi = random.randint(-100,100)
		if(1 in choice):
			if(arr[i] == mini):
...
def diag_tl_br(arr):
	"""
	Generate diagonal grating (top left to bottom right)
 	"""
	phase_choice = random.randint(0,1)
	arr[:] = abs(phase_choice - 1) * 255
...
def diag_tr_bl(arr):
	"""
	Generate diagonal grating (top right to bottom left)
	"""
	phase_choice = random.randint(0,1)
	arr[:] = abs(phase_choice - 1) * 255
...
def generate_data_final():
	"""
	Generates data in the format [[input for contrast, output for contrast], [inpyt for orientation, otput for orientation]]
	note: Input is same for both
 	"""
	seq_l = 16
	input_size = 64
...
class RNN_arch_2_1(nn.Module):
	"""
	Class for RNN module for contrast and orientation networks
 	"""
	def __init__(self, batch_size):
		super().__init__()
		self.batch_size = batch_size
		self.rnn = nn.RNN(64, 64, num_layers =1, bias = False, nonlinearity='tanh')
	...
	def forward(self, x, hc):
		...
		r_output = out.contiguous().view(-1, 64)
...
class RNN_arch_2_final(nn.Module):
	"""
 	Class for RNN module for entire network
	"""
	def __init__(self, batch_size):
		super().__init__()
		self.batch_size = batch_size

		self.rnn1_i2h = nn.Linear(64, 64)
		self.rnn1_h2h = nn.Linear(64,64)
		self.tanh1 = nn.Tanh()
		self.feedback1 = nn.Linear(64,64)
		self.tanh2 = nn.Tanh()
...
class RNN_decoder(nn.Module):
	"""
	Class for RNN module for decoder
	"""
	def __init__(self, batch_size):
		super().__init__()
		self.batch_size = batch_size
...
def weight_init(m):
	"""
	To initialise weights of linear layers
	"""
...
def init_weights(model):
	"""
	To initialise weights of RNN layers
	"""
...
def train():
	...
	while(count0<2000):
		...
		torch.save(model.state_dict(), "decoder_motion_feedback_finalTask.pt")

# thread = Thread(target=train)
# thread.start()

# decoder_motion_feedback_initialTask
# decoder_contrast_feedback_actualInitialTask
# decoder_contrast_feedback_initialTask (for final task)
# decoder_motion_feedback_finalTask